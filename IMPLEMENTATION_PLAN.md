...
- [x] **BUG-FIX B-42**: Hydration mismatch — MUI CssBaseline creates `<style data-emotion>` SSR stylesheet that differs from client-side HTML. Fix: add `AppRouterCacheProvider` from `@mui/material-nextjs/v13-appRouter` in `layout.tsx` wrapping `AppThemeProvider`. Revert hack from `app-theme-provider.tsx`. Also installed `@mui/material-nextjs` package.
- [x] **BUG-FIX B-43**: Search filter giật lag / removed — Đã xoá hoàn toàn: FilterBar không còn search TextField, `search`/`deferredSearch`/`useDeferredValue` state đã xoá khỏi dashboard-client.tsx, filteredMembers và filteredTasks không còn search filter, tasksQuery không còn search param.
...
- [x] **FEATURE F-02**: Dashboard chart member names hidden — Recharts XAxis mặc định ẩn label khi >~5 items. Fix: đổi cả 2 BarChart sang `layout="vertical"` (horizontal bars), YAxis là `type="category"` hiển thị tên member, XAxis là giá trị số. Không cần xoay text, tất cả tên hiển thị đầy đủ.
