...
export async function generateExcelReport(reportType, periodStart, periodEnd, rows) {
...
export async function generatePdfReport(reportType, periodStart, periodEnd, performance) {
    await ensureExportDir();
    const doc = new jsPDF();
    doc.setFontSize(14);
    ...
    for (const row of performance) {
        doc.text(`${row.memberName}: score=${row.score}, avgDelay=${row.avgDelayDays}, overdueRatio=${row.overdueRatio}`, 14, y);
        y += 8;
        if (y > 270) {
            doc.addPage();
            y = 20;
