...
app.post("/members", requireRoles(["admin", "pm", "lead"]), (req, res) => {
    ...
    const duplicated = getMembersWithSeed().some((member) => member.email === parsed.data.email || member.memberCode === parsed.data.memberCode);
    if (duplicated)
        return res.status(409).json({ error: "Duplicate member_code or email" });
...
app.post("/projects", requireRoles(["admin", "pm", "lead"]), (req, res) => {
    ...
    if (duplicated)
        return res.status(409).json({ error: "Duplicate projectCode" });
