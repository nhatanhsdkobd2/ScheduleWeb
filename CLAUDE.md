...
| B-42 | Hydration mismatch: MUI CssBaseline tạo `<style data-emotion>` trên server nhưng client nhận `<div>` | HIGH | MUI Emotion SSR stylesheet không đồng bộ giữa server-rendered HTML và client hydration — gây "Recoverable Error" hydration mismatch | ✅ Đã fix: thêm `AppRouterCacheProvider` từ `@mui/material-nextjs/v13-appRouter` vào `layout.tsx` (bọc ngoài `AppThemeProvider`), xoá hack trong `app-theme-provider.tsx`. |
| B-43 | Search filter giật lag mỗi keystroke / removed | MEDIUM | `onChange` gọi `setSearch` trực tiếp → state update mỗi keystroke → re-render toàn bộ component → giật lag | ✅ Đã xoá hoàn toàn: FilterBar không còn search TextField, `search`/`deferredSearch` state đã xoá khỏi dashboard-client. |
| F-01 | Tasks tab: nút Export CSV | MEDIUM | Tasks tab có "Add task" nhưng thiếu nút Export — người dùng cần export danh sách task đã filter ra file CSV | ✅ Đã fix: thêm Button "Export CSV" bên cạnh "Add task" trong tab Tasks. Export dựa trên `taskTableRows` (đã filter). UTF-8 with BOM, escape dấu phẩy/double-quote trong CSV. |
| F-02 | Dashboard: chart không hiển thị tên member | MEDIUM | Performance Score by Member và Workload by Member chart không hiển thị tên dưới mỗi cột — Recharts XAxis mặc định ẩn label khi có nhiều items | ✅ Đã fix: đổi cả 2 chart sang `layout="vertical"` (horizontal bars), YAxis là `type="category"` hiển thị tên member, XAxis là giá trị số. Không cần xoay text, tất cả tên hiển thị đầy đủ. |
